...
FROM fsm_analytics_specialized.pathology_fact.pathology_case pc JOIN nm_bi.dim.vw_patient p ON pc.patient_ir_id = p.ir_id AND p.is_current = 1
                                                                JOIN FSM_Analytics_Specialized.pathology_dim.pathology_case_group g ON pc.pathology_case_group_key = g.pathology_case_group_key
...
--pc.accessioned_datetime >= '3/1/2019' AND pc.accessioned_datetime < '4/1/2019'
WHERE g.source_system_id IN(
...
SELECT  orc.or_case_id
      ...
      , (SELECT count(*) FROM FSM_Analytics_Specialized.pathology_map.pathology_and_surgical_case_map map WHERE map.pathology_case_key = pc.pathology_case_key AND map.surgical_case_key = sc.surgical_case_key) AS present_map_count
      , pc.group_name
      , pc.group_desc
      , pc.snomed_code
      , pc.snomed_name
...
FROM clarity.dbo.OR_CASE orc JOIN #pathology_cases pc ON pc.clarity_west_id = orc.pat_id AND(CAST(orc.surgery_date AS DATE) = CAST(pc.case_collect_datetime AS DATE) OR CAST(orc.surgery_date AS DATE) = CAST(pc.accessioned_datetime AS DATE))
                             LEFT JOIN nm_bi.fact.vw_surgical_case sc ON orc.or_case_id = sc.surgical_case_number


SELECT DISTINCT
                p.west_mrn
                , #cohort.patient_ir_id
                ,  orc.surgery_date
                ,  orc.or_case_id
                ,  prov1.full_name AS primary_surgeon_full_name
                ,  prov1.npi       AS primary_surgeon_npi
                ,  #pathology_cases.source_system_id
                ,  #pathology_cases.accession_nbr_formatted
                ,  #pathology_cases.case_collect_datetime
                ...
                ,  n.note_type
                ,  n.note_text
FROM clarity.dbo.OR_CASE orc  JOIN clarity.dbo.ZC_OR_SERVICE ser on ser.service_c=orc.SERVICE_C
                              JOIN nm_bi.fact.vw_surgical_case sc ON orc.or_case_id = sc.surgical_case_number
                              JOIN nm_bi.dim.provider prov1 ON sc.primary_surgeon_provider_key = prov1.provider_key
                              JOIN nm_bi.dim.vw_patient p ON p.clarity_west_id = orc.PAT_ID
                              LEFT JOIN #pathology_cases_surgery_mappings ON orc.or_case_id = #pathology_cases_surgery_mappings.or_case_id
                              LEFT JOIN #pathology_cases ON #pathology_cases.pathology_case_key = #pathology_cases_surgery_mappings.pathology_case_key
                              LEFT JOIN fsm_analytics_specialized.pathology_fact.pathology_case_report prc ON #pathology_cases.pathology_case_key = prc.pathology_case_key
                              LEFT JOIN fsm_analytics_specialized.pathology_fact.pathology_case_report_section prcs ON prc.pathology_case_report_key = prcs.pathology_case_report_key AND prcs.section_description = 'Final Diagnosis'
                              LEFT JOIN nm_bi.fact.vw_note n ON prcs.note_key = n.note_key
                              LEFT JOIN #cohort ON p.ir_id = #cohort.patient_ir_id
WHERE ser.NAME IN (
'Neurosurgery',
'Neurospine',
...
AND orc.surgery_date >= '3/1/2019' AND orc.surgery_date < '4/1/2019'
--AND #pathology_cases.source_system_id = '40505456'
AND  orc.or_case_id = '639974'
