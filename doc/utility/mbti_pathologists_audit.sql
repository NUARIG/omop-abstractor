...
FROM clarity.dbo.OR_CASE orc JOIN #pathology_cases pc ON pc.clarity_west_id = orc.pat_id AND CAST(orc.surgery_date AS DATE) = CAST(pc.case_collect_datetime AS DATE)
                             ...
                             LEFT JOIN nm_bi.dim.provider prov1 ON sc.primary_surgeon_provider_key = prov1.provider_key

SELECT DISTINCT responsible_pathologist_full_name
FROM(
SELECT   #pathology_cases.source_system_id
      ...
      ,  n.note_type
      ,  n.note_text
FROM #pathology_cases    JOIN fsm_analytics_specialized.pathology_fact.pathology_case_report prc ON #pathology_cases.pathology_case_key = prc.pathology_case_key
                         JOIN fsm_analytics_specialized.pathology_fact.pathology_case_report_section prcs ON prc.pathology_case_report_key = prcs.pathology_case_report_key
...
AND prcs.section_description = 'Final Diagnosis'
--AND #pathology_cases.source_system_id = '40505456'
) data
ORDER BY data.responsible_pathologist_full_name
