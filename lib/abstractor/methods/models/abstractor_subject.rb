    ...
    module Models
      module AbstractorSubject
        ...
        module InstanceMethods
          ...
          # The method will create instances of Abstractor::AbstractorAbstraction and
          ...
          def abstract_not_applicable(about, abstractor_abstraction, abstractor_abstraction_source)
            create_not_applicable_abstractor_suggestion(about, abstractor_abstraction, abstractor_abstraction_source)
          end
          ...
          def abstract_canonical_name_value(about, abstractor_abstraction, abstractor_abstraction_source)
            abstractor_abstraction_source.normalize_from_method_to_sources(about).each do |source|
              ...
              abstractor_abstraction_schema.predicate_variants.each do |predicate_variant|
                ...
                when Abstractor::Enum::ABSTRACTOR_OBJECT_TYPE_LIST, Abstractor::Enum::ABSTRACTOR_OBJECT_TYPE_BOOLEAN, Abstractor::Enum::ABSTRACTOR_OBJECT_TYPE_RADIO_BUTTON_LIST, Abstractor::Enum::ABSTRACTOR_OBJECT_TYPE_DYNAMIC_LIST, Abstractor::Enum::ABSTRACTOR_OBJECT_TYPE_NUMBER_LIST
                ...
                when Abstractor::Enum::ABSTRACTOR_OBJECT_TYPE_NUMBER
                  match_value = "#{Regexp.escape(predicate_variant)}:\s*\d+"
                  matches = parser.scan(match_value, word_boundary: true).uniq
                  matches.each do |match|
                    suggest(abstractor_abstraction, abstractor_abstraction_source, match, match, source[:source_id], source[:source_type].to_s, source[:source_method], source[:section_name],  abstractor_object_value, nil, nil, nil, nil)
                  end
                ...
                when Abstractor::Enum::ABSTRACTOR_OBJECT_TYPE_NUMBER
                  ...
                  if ranges.any?
                    ranges.each do |range|
                      sentence = parser.find_sentence(range)
                      if sentence
                        numbers = sentence[:sentence].scan(/\d+/)
                        numbers.each do |number|
                          scoped_sentence = Abstractor::NegationDetection.parse_negation_scope(sentence[:sentence])
                          reject = (
                                     Abstractor::NegationDetection.negated_match_value?(scoped_sentence[:scoped_sentence], predicate_variant) ||
                                     Abstractor::NegationDetection.manual_negated_match_value?(sentence[:sentence], predicate_variant) ||
                                     Abstractor::NegationDetection.negated_match_value?(scoped_sentence[:scoped_sentence], number) ||
                                     Abstractor::NegationDetection.manual_negated_match_value?(sentence[:sentence], number)
                                   )
                          if !reject
                            suggest(abstractor_abstraction, abstractor_abstraction_source, sentence[:sentence], sentence[:sentence], source[:source_id], source[:source_type].to_s, source[:source_method], source[:section_name], number, nil, nil, nil, nil)
                          end
                        end
                      end
                    end
          ...
          def create_unknown_abstractor_suggestion_name_only(about, abstractor_abstraction, abstractor_abstraction_source)
            abstractor_abstraction_source.normalize_from_method_to_sources(about).each do |source|
              ...
              if abstractor_abstraction.abstractor_suggestions.reload.empty?
                abstractor_abstraction_schema.predicate_variants.each do |predicate_variant|
                  ranges = parser.range_all(Regexp.escape(predicate_variant))
                  if ranges
                    ranges.each do |range|
          ...
          def create_not_applicable_abstractor_suggestion(about, abstractor_abstraction, abstractor_abstraction_source)
            abstractor_abstraction_source.normalize_from_method_to_sources(about).each do |source|
              suggest(abstractor_abstraction, abstractor_abstraction_source, nil, nil, source[:source_id], source[:source_type].to_s, source[:source_method],source[:section_name],  nil, nil, true, nil, nil)
            end
          end
