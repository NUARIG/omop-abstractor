...
namespace :clamp do
  ...
  desc "Load schemas Oncoset"
  task(schemas_oncoset: :environment) do |t, args|
    ...
    primary_histologies.each do |histology|
      ...
      histology_synonyms = Icdo3Histology.by_primary_and_icdo3_code_with_synonyms(histology.icdo3_code)
      histology_synonyms.each do |histology_synonym|
  ...
  desc "Clamp Dictionary Oncoset"
  task(clamp_dictionary_oncoset: :environment) do  |t, args|
    dictionary_items = []
    ...
    File.open 'lib/setup/data_out/oncoset_abstractor_clamp_data_dictionary.txt', 'w' do |f|
