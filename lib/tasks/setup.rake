...
namespace :setup do
  ...
  desc 'Load schemas new'
  task(schemas_new: :environment) do |t, args|
    date_object_type = Abstractor::AbstractorObjectType.where(value: 'date').first
    list_object_type = Abstractor::AbstractorObjectType.where(value: 'list').first
    boolean_object_type = Abstractor::AbstractorObjectType.where(value: 'boolean').first
    string_object_type = Abstractor::AbstractorObjectType.where(value: 'string').first
  ...
  task(clamp_dictionary: :environment) do  |t, args|
    dictionary_items = []
    # 'has_metastatic_cancer_site'
    Abstractor::AbstractorAbstractionSchema.where(predicate: ['has_cancer_site', 'has_cancer_histology', 'has_metastatic_cancer_histology', 'has_metastatic_cancer_primary_site']).all.each do |abstractor_abstraction_schema|
  ...
  task(map_clamp_to_abstractor: :environment) do  |t, args|
    Dir.glob("#{Rails.root}/lib/setup/data_out/custom_nlp_provider_clamp/*.xmi").each do |file|
      ...
      clamp_note = parser.read(file).document
      abstractor_note = File.read(file) { |f| JSON.parse(f) }
