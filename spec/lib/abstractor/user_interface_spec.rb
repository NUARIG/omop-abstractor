...
describe Abstractor::UserInterface do
  it "prepends a relative url if the path does not include the root and is set ", focus: false do
    allow(OmopAbstractor::Application.config.action_controller).to receive(:relative_url_root).and_return('/dummy')
  ...
  it "does not prepend a relative url root if is not set", focus: false do
    allow(OmopAbstractor::Application.config.action_controller).to receive(:relative_url_root).and_return(nil)
