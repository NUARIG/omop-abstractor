...
#     @abstractor_subject_abstraction_schema_imaging_confirmed_extent_of_resection = Abstractor::AbstractorSubject.where(subject_type: NoteStableIdentifier.to_s, abstractor_abstraction_schema_id: @abstractor_abstraction_schema_imaging_confirmed_extent_of_resection.id).first
...
#   end
#
#   it "creates a 'has_imaging_confirmed_extent_of_resection' abstraction for an abstractor abstraction source type 'indirect'", focus: false do
#     pending "Expected to fail: Need to revisit support for indirect sources."
#     @note_stable_identifier.abstract
#     expect(@note_stable_identifier.reload.detect_abstractor_abstraction(@abstractor_subject_abstraction_schema_imaging_confirmed_extent_of_resection)).to_not be_nil
#   end
#
...
#
#     source_source_method_modifier = {:source_type=>
#            SurgicalProcedure,
#           :source_id=>surgical_procedure.id,
...
#            ]}
#
#     source_source_method_description = {:source_type=>
#        SurgicalProcedure,
#       :source_id=>surgical_procedure.id,
...
#       ]}
#
#     expect(surgery.reload.sources.select { |source| source[:source_method] == "modifier" }.first).to eq(source_source_method_modifier)
#     expect(surgery.reload.sources.select { |source| source[:source_method] == "description" }.first).to eq(source_source_method_description)
#   end
#
